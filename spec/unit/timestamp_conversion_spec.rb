...
RSpec.describe InfluxDB::Rails::TimestampConversion do
  let(:test_class) { Class.new { include InfluxDB::Rails::TimestampConversion } }
  ...
  describe ".convert_timestamp" do
...

    {
      ...
      it "should return the timestamp in #{precision.inspect}" do
        expect(subject.convert_timestamp(sometime, precision)).to eq(converted_value)
      end
    end

    it "should raise an excpetion when precision is unrecognized" do
      expect { subject.convert_timestamp(sometime, "whatever") }
